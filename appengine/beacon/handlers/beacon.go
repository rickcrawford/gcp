package handlers

import (
	"context"
	"encoding/base64"
	"net/http"
	"strconv"
	"strings"
	"time"

	"github.com/go-chi/chi"
	"google.golang.org/appengine"
	"google.golang.org/appengine/delay"
	"google.golang.org/appengine/log"
	"google.golang.org/appengine/taskqueue"

	"github.com/rickcrawford/beacon/managers"
)

const (
	emptyPNG  = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg=="
	emptyGIF  = "R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="
	emptyMP3  = ""
	emptySVG  = ""
	emptySWF  = "Q1dTCGYAAAB4nKtgYI1nYOBfwMDAw8jgIsgABM5M/4GAkwPIZARCByBtzycH5QUk5qXmxJdkluSkOiUmZ6cX5ZfmpQRnZ+Yx9LGxgRQc/f/h/3/T6L6JIH0ApWwYMg=="
	emptyJS   = "// nocontent\n"
	emptyCSS  = "/* nocontent */\n"
	emptyMP4  = ""
	emptyOGG  = ""
	emptyWEBM = ""
	emptyFLV  = "RkxWAQAAAAAJAAAAAA=="

	maxSkewSeconds = 30

	deferFuncKey = "request-recorder"
)

var record = delay.Func(deferFuncKey, func(ctx context.Context, fileType string, r *http.Request) {
	log.Infof(ctx, "recording event: %s", r.URL.String())
	if err := managers.NewBeaconManager().Save(ctx, fileType, r, nil); err != nil {
		log.Errorf(ctx, "Error saving request: %s", err)
	}
})

type beaconHandler struct {
}

func (b beaconHandler) get(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	status := http.StatusOK

	// close the request body to release it...
	defer r.Body.Close()

	w.Header().Set("Cache-Control", "no-cache")
	w.Header().Set("Pragma", "no-cache")
	w.Header().Set("Expires", time.Time{}.Format(time.RFC822))

	var data []byte

	fileType := strings.ToLower(chi.URLParam(r, "ext"))
	switch fileType {
	case "gif":
		w.Header().Set("Content-Type", "image/gif")
		data, _ = base64.StdEncoding.DecodeString(emptyGIF)

	case "png":
		w.Header().Set("Content-Type", "image/png")
		data, _ = base64.StdEncoding.DecodeString(emptyPNG)

	case "js":
		w.Header().Set("Content-Type", "application/javascript")
		data = []byte(emptyJS)

	case "css":
		w.Header().Set("Content-Type", "text/css")
		data = []byte(emptyCSS)

	case "svg":
		w.Header().Set("Content-Type", "image/svg+xml")
		data = []byte(emptySVG)

	case "mp3":
		w.Header().Set("Content-Type", "audio/mpeg")
		data, _ = base64.StdEncoding.DecodeString(emptyMP3)

	case "swf":
		w.Header().Set("Content-Type", "application/x-shockwave-flash")
		data, _ = base64.StdEncoding.DecodeString(emptySWF)

	case "m4v", "mp4":
		w.Header().Set("Content-Type", "video/mp4")
		data, _ = base64.StdEncoding.DecodeString(emptyMP4)

	case "ogg":
		w.Header().Set("Content-Type", "video/ogg")
		data, _ = base64.StdEncoding.DecodeString(emptyOGG)

	case "webm":
		w.Header().Set("Content-Type", "video/webm")
		data, _ = base64.StdEncoding.DecodeString(emptyWEBM)

	case "flv":
		w.Header().Set("Content-Type", "video/x-flv")
		data, _ = base64.StdEncoding.DecodeString(emptyFLV)

	default:
		status = http.StatusNoContent
	}

	w.WriteHeader(status)
	w.Write(data)

	// hostname -- bestbuy.com
	hostName := strings.TrimSpace(strings.ToLower(r.FormValue("_h")))
	if hostName == "" {
		log.Warningf(ctx, "invalid hostname: %s", hostName)
		return
	}

	// must have an event time
	eventTimeStr := r.FormValue("_t")
	eventTime, err := strconv.ParseInt(eventTimeStr, 10, 64)
	if err != nil || eventTime == 0 {
		log.Warningf(ctx, "invalid time: %s", eventTimeStr)

		return
	}

	// check the skew... if > skew then this time is outside our range
	now := time.Now().UnixNano()
	skew := now - (eventTime * 1000000)
	if skew/1000000000 >= maxSkewSeconds {
		log.Warningf(ctx, "invalid skew: %d", skew/1000000000)
		return
	}

	//TODO: this is a hack because the default app uses the delay queue...
	// since we have a custom domain it breaks it.  We have to reset the hostname here.
	// we should really create a struct of the request values we want that will gob encode ok.

	// create  a shallow copy of the request, dicard the body.
	rc := new(http.Request)
	*rc = *r
	rc.Body = nil

	t, err := record.Task(fileType, rc)
	if err != nil {
		log.Errorf(ctx, "Error creating task: %s", err)
		return
	}

	t.Header = http.Header{}
	aeHostname, err := appengine.ModuleHostname(ctx, "beacon", "", "")
	if err != nil {
		log.Errorf(ctx, "Error getting hostname: %s", err)
		return
	}
	t.Header.Set("Host", aeHostname)
	_, err = taskqueue.Add(ctx, t, "")
	if err != nil {
		log.Errorf(ctx, "Error adding task to queue: %s", err)
	}
}

func (b beaconHandler) post(w http.ResponseWriter, r *http.Request) {
	status := http.StatusOK
	w.WriteHeader(status)
}

func (b beaconHandler) warmup(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	if err := managers.NewBeaconManager().Init(r.Context()); err != nil {
		log.Errorf(ctx, "error running warmup: %s", err)
	}
	log.Infof(ctx, "warmup run...")

}
